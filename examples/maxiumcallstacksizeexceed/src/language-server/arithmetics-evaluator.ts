 ...
 ******************************************************************************/
import type { AbstractDefinition, BinaryExpression, Definition, Evaluation, Expression, FunctionDeclaration, Module, Statement } from './generated/ast.js';
...
function evalStatement(ctx: InterpreterContext, stmt: Statement): void {
    if (isDefinition(stmt)) {
        if (stmt.$type === 'FunctionDeclaration') {
            evalDefinition(ctx, stmt as FunctionDeclaration);
        }
...
function evalDefinition(ctx: InterpreterContext, def: FunctionDeclaration): void {
...
export function evalExpression(expr: Expression, ctx?: InterpreterContext): number {
    ...
    if (isFunctionCall(expr)) {
        let valueOrDef = ctx.context.get((expr.func.ref as AbstractDefinition).name) as number | Definition;
        if (!isDefinition(valueOrDef) || (valueOrDef.$type === 'FunctionDeclaration')) {
            return 0;
        }
        valueOrDef = valueOrDef as unknown as FunctionDeclaration;
